...
class PineconeClient(object):
    ...
    def store_embeddings(self, embeddings, index_name, dimension, namespace=None):
        ...
        pinecone_index.upsert(vectors=embeddings, namespace=namespace)



    def search_pinecone(self, embeddings, index_name, namespace=None):

        # Connect to the Pinecone index
        index = pinecone.Index(index_name)

        # Perform a nearest neighbor search in Pinecone
        query_results = index.query(vector=embeddings, namespace=namespace, top_k=1, include_values=False, include_metadata=True)
